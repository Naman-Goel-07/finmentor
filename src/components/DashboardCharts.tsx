...
export default function DashboardCharts({ expenses }: { expenses: any[] }) {
	...
	const barData = Object.keys(monthMap)
		.map((key) => ({
			name: key,
			Total: monthMap[key],
		}))
					...
					{pieData.length > 0 ? (
						<ResponsiveContainer width="100%" height="100%">
							<PieChart>
								...
								{/* ✅ UPDATED: Tooltip formatter to use ₹ and Indian formatting */}
								<Tooltip formatter={(value) => `₹${Number(value ?? 0).toLocaleString('en-IN', { minimumFractionDigits: 2 })}`} />
								<Legend />
							</PieChart>
						</ResponsiveContainer>
					) : (
					...
					{barData.length > 0 ? (
						<ResponsiveContainer width="100%" height="100%">
							<BarChart data={barData}>
								...
								{/* ✅ UPDATED: YAxis tickFormatter to use ₹ */}
								<YAxis
									axisLine={false}
									tickLine={false}
									tick={{ fill: '#64748b' }}
								...
								{/* ✅ UPDATED: Bar Tooltip formatter to use ₹ */}
								<Tooltip
									cursor={{ fill: '#f8fafc' }}
